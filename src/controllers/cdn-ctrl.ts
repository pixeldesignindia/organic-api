...
export default class CDNController extends BaseController {
    ...
    public initializeRoutes() {
        this.router.get(constants.API.V1 + constants.API.APP.CDN + '/user-image/:file', (req, res) => { this.getUserImage(req, res, this) });
        this.router.get(constants.API.V1 + constants.API.APP.CDN + '/product-image/:file', (req, res) => { this.getProductImage(req, res, this) });
    }

    private getUserImage(req: Request, res: Response, that: any) {
        this.service.getUserImage(req.params, req.headers).then((result: any) => {
    ...
    private getProductImage(req: Request, res: Response, that: any) {
        this.service.getProductImage(req.params, req.headers).then((result: any) => {
            that.responseUtil.sendTextResponse(req, res, result, 200);
        }, (err: any) => {
