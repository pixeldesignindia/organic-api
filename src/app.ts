	...
	public initializeMiddlewares() {
		...
		this.app.use((request: express.Request, response: express.Response, next: any) => {
			...
			response.setHeader(
				...
				'DNT,X-Mx-ReqToken,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type,authToken,loggedusertype,loggeduserid,clinicid'
			);
			next();
		});
	...
	public initializeJwtVerification() {
		...
		this.app.use((req, res, next) => {
			if (req.url.indexOf('/api/v1/') > -1) {
				...
				if (req.url.indexOf('/api/v1/') > -1) urlToCheck = req.url.split('/api/v1/')[1];
